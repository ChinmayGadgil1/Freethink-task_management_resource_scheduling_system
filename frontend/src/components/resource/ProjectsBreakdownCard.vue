  ...
  <q-card flat bordered class="dashboard-card">
    ...
    <div v-for="row in projects" :key="row.project" class="project-row">
      <div class="project-main">
        ...
        <div class="text-caption text-grey-6">{{ row.tasks }} assigned tasks · Due {{ row.deadline }}</div>
      </div>

      <q-linear-progress
        :value="row.progress / 100" size="7px" rounded class="project-progress"
...
.project-name { font-size: 13px; font-weight: 600; color: var(--wo-text-main, #1d2433); }
...
.status-ok { background: #eaf7f0; color: #27ae60; }
.status-warn { background: #fff4e8; color: #e89532; }
.status-bad { background: #fdeef0; color: #e15263; }

@media (max-width: 700px) {
  .project-row { grid-template-columns: 1fr; align-items: flex-start; }
  .project-side { justify-content: flex-start; }
}
</style>
