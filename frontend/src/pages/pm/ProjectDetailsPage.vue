    ...
    <div class="q-mx-auto column q-gutter-y-lg" style="max-width: 1400px">
        ...
        <div class="row q-col-gutter-lg justify-between items-start">
          <div class="col-12 col-md-8 column q-gutter-y-md">
            ...
            <div class="row items-center q-gutter-xs wrap">
              <q-chip dense square :color="statusColor(project.status)" :text-color="statusTextColor(project.status)" class="text-caption text-weight-bold">
                {{ formatStatus(project.status) }}
              </q-chip>
              <q-chip dense square :color="healthColor" :text-color="healthTextColor" class="text-caption text-weight-bold">
                <q-icon :name="projectHealth === 'ON_TRACK' ? 'check_circle' : projectHealth === 'AT_RISK' ? 'warning' : 'schedule'" size="13px" class="q-mr-xs" />
                {{ healthLabel }}
              </q-chip>
              <q-chip dense square :color="priorityColor(project.priority)" :text-color="priorityTextColor(project.priority)" class="text-caption text-weight-bold">
            ...
            <!-- Title & Description -->
            <div>
              <div class="text-h4 text-weight-bolder" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">{{ project.name }}</div>
              <div class="text-body2 q-mt-xs" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-7'">
                {{ project.description || 'Comprehensive project plan tracking milestones, resource allocations, and task deliverables across engineering and product teams.' }}
              </div>
            </div>
            ...
            <div class="row q-col-gutter-md q-mt-xs">
              <div class="col-6 col-sm-3">
                <div class="text-caption" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">Project Manager</div>
                <div class="row items-center q-gutter-xs q-mt-xs">
                  <q-avatar size="22px" color="primary" text-color="white" class="text-weight-bold">{{ currentPmName.charAt(0) }}</q-avatar>
                  <span class="text-caption text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">{{ currentPmName }}</span>
                </div>
              </div>

              <div class="col-6 col-sm-3">
                <div class="text-caption" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">Timeline</div>
                <div class="text-caption text-weight-bold q-mt-xs" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">
                  {{ formatDate(project.start_date) }} — {{ formatDate(project.deadline) }}
                </div>
              </div>

              <div class="col-6 col-sm-3">
                <div class="text-caption" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">Days Remaining</div>
                <div class="text-caption text-weight-bold q-mt-xs" :class="daysRemaining < 0 ? 'text-negative' : 'text-primary'">
                  {{ daysRemainingText }}
                </div>
              </div>

              <div class="col-6 col-sm-3">
                <div class="text-caption" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">Team Members</div>
                <div class="row items-center q-gutter-xs q-mt-xs">
                  <q-avatar v-for="m in teamMembers.slice(0, 3)" :key="m.id" size="24px" color="primary" text-color="white" class="text-caption text-weight-bold">
                    {{ m.name.charAt(0) }}
                  </q-avatar>
                  <q-chip v-if="teamMembers.length > 3" dense square color="grey-3" text-color="dark" class="text-caption text-weight-bold">
          ...
          <div class="col-12 col-md-4 column q-gutter-y-md">
            <q-card flat bordered :dark="$q.dark.isActive" class="rounded-borders q-pa-md">
              <div class="row items-center justify-between">
                <span class="text-caption text-weight-bold" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-7'">Overall Completion</span>
            ...
            <div class="row items-center q-gutter-sm">
              <q-btn unelevated no-caps color="primary" icon="add_task" label="Add Task" class="col text-weight-bold" @click="showCreateTaskDialog = true" />
              <q-btn outline no-caps :color="$q.dark.isActive ? 'grey-4' : 'grey-8'" icon="edit" label="Edit" class="q-px-md text-weight-bold" @click="openEditDialog" />
              <q-btn flat round dense icon="more_vert" :color="$q.dark.isActive ? 'grey-4' : 'grey-7'">
                <q-menu auto-close>
                  <q-list style="min-width: 170px">
                    <q-item clickable @click="refreshData"><q-item-section avatar><q-icon name="refresh" size="18px" /></q-item-section><q-item-section>Refresh Data</q-item-section></q-item>
                    <q-item clickable @click="exportProjectSummary"><q-item-section avatar><q-icon name="download" size="18px" /></q-item-section><q-item-section>Export Report</q-item-section></q-item>
                    <q-separator />
                    <q-item clickable @click="markProjectComplete"><q-item-section avatar><q-icon name="task_alt" size="18px" color="positive" /></q-item-section><q-item-section class="text-positive">Mark Complete</q-item-section></q-item>
                    <q-separator />
                    <q-item clickable class="text-negative" @click="confirmDeleteProject"><q-item-section avatar><q-icon name="delete" size="18px" color="negative" /></q-item-section><q-item-section>Delete Project</q-item-section></q-item>
                  </q-list>
                </q-menu>
              </q-btn>
            </div>
      ...
      <div class="row q-col-gutter-md">
        <div class="col-12 col-sm-6 col-md-2">
          <StatCard title="Overall Progress" :value="`${overallProgress}%`" :subtitle="`${completedTasksCount} completed`" icon="donut_large" color="purple" note-class="note-green" />
        </div>
        <div class="col-12 col-sm-6 col-md-2">
          <StatCard title="Total Tasks" :value="totalTasksCount" :subtitle="`${inProgressTasksCount} in progress`" icon="task_alt" color="teal" note-class="note-teal" />
        </div>
        <div class="col-12 col-sm-6 col-md-2">
          <StatCard title="Completed Tasks" :value="completedTasksCount" :subtitle="`${taskCompletionRate}% done`" icon="check_circle" color="green" note-class="note-green" />
        </div>
        <div class="col-12 col-sm-6 col-md-2">
          <StatCard title="Overdue Tasks" :value="overdueTasksCount" :subtitle="overdueTasksCount > 0 ? 'Requires attention' : 'All on schedule'" icon="schedule" color="red" :note-class="overdueTasksCount > 0 ? 'note-red' : 'note-green'" :negative="overdueTasksCount > 0" />
        </div>
        <div class="col-12 col-sm-6 col-md-2">
          <StatCard title="Team Members" :value="teamMembers.length" :subtitle="`${activeAssigneesCount} assigned`" icon="groups" color="orange" note-class="note-orange" />
        </div>
        <div class="col-12 col-sm-6 col-md-2">
          <StatCard title="Days Remaining" :value="daysRemaining > 0 ? daysRemaining : daysRemaining === 0 ? 'Due today' : Math.abs(daysRemaining) + 'd ago'" :subtitle="`Due ${formatDate(project.deadline)}`" icon="event_available" color="blue" note-class="note-blue" :negative="daysRemaining < 0" />
        </div>
      </div>
          ...
          <q-card flat bordered :dark="$q.dark.isActive" class="rounded-borders full-height">
            <q-card-section class="row items-center justify-between">
              <div class="row items-center">
                ...
                <div class="text-subtitle1 text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">Progress & Effort Breakdown</div>
              </div>
              <q-chip dense square :color="statusColor(project.status)" :text-color="statusTextColor(project.status)" class="text-caption text-weight-bold">
                {{ formatStatus(project.status) }}
              </q-chip>
            ...
            <q-card-section class="column q-gutter-y-md">
              <div class="row q-col-gutter-xs text-center">
                ...
                <div class="col-3">
                  ...
                  <div class="text-h6 text-weight-bold text-positive">{{ completedTasksCount }}</div>
                </div>
                <div class="col-3">
              ...
              <!-- Multi Segmented Bar -->
              <div class="row no-wrap rounded-borders overflow-hidden" style="height: 10px; background: rgba(200,200,200,0.2);">
                <div :style="{ width: `${totalTasksCount ? (completedTasksCount / totalTasksCount) * 100 : 0}%`, background: '#27ae60' }" />
                <div :style="{ width: `${totalTasksCount ? (inProgressTasksCount / totalTasksCount) * 100 : 0}%`, background: '#2e90fa' }" />
                <div :style="{ width: `${totalTasksCount ? (onHoldTasksCount / totalTasksCount) * 100 : 0}%`, background: '#f5841f' }" />
                <div :style="{ width: `${totalTasksCount ? (pendingTasksCount / totalTasksCount) * 100 : 0}%`, background: '#8b6fd8' }" />
              </div>
              ...
              <q-card flat bordered :dark="$q.dark.isActive" class="rounded-borders q-pa-md">
                <div class="row items-center justify-between text-caption">
                  <div>
                    <span :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">Logged Effort: </span>
                    <strong :class="$q.dark.isActive ? 'text-white' : 'text-dark'">{{ totalEffortLogged }} Hours</strong>
                  </div>
                  <div>
                    <span :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">Expected Effort: </span>
                    <strong :class="$q.dark.isActive ? 'text-white' : 'text-dark'">{{ totalEffortExpected }} Hours</strong>
                  </div>
                </div>
                ...
                <q-linear-progress
                  rounded
                  size="6px"
                  :value="totalEffortExpected ? Math.min(1, totalEffortLogged / totalEffortExpected) : 0"
                  color="primary"
                  class="q-mt-sm"
                  :track-color="$q.dark.isActive ? 'grey-9' : 'grey-3'"
                />
          ...
          <q-card flat bordered :dark="$q.dark.isActive" class="rounded-borders full-height">
            <q-card-section class="row items-center justify-between">
              <div class="row items-center">
                ...
                <div class="text-subtitle1 text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">Project Milestones</div>
              </div>
            ...
            <q-list v-if="milestones.length" separator :dark="$q.dark.isActive">
              <q-item v-for="m in milestones" :key="m.id" class="q-py-md">
                <q-item-section avatar>
                  <q-avatar size="28px" :color="m.progress === 100 ? 'positive' : m.progress > 0 ? 'primary' : 'grey-5'" text-color="white">
                    <q-icon :name="m.progress === 100 ? 'check' : m.progress > 0 ? 'sync' : 'hourglass_empty'" size="14px" />
                  </q-avatar>
                </q-item-section>

                <q-item-section>
                  <div class="row items-center justify-between">
                    <span class="text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">{{ m.name }}</span>
                    <q-chip dense square color="primary" text-color="white" class="text-caption text-weight-bold">
                      {{ m.status.replace('_', ' ') }}
                    </q-chip>
                  </div>

                  <q-linear-progress :value="m.progress / 100" color="primary" rounded size="5px" class="q-mt-xs" :track-color="$q.dark.isActive ? 'grey-9' : 'grey-3'" />

                  <div class="row items-center justify-between text-caption q-mt-xs" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">
      ...
      <q-card flat bordered :dark="$q.dark.isActive" class="rounded-borders">
        <q-card-section class="row items-center justify-between wrap q-gutter-y-sm">
          <div class="row items-center">
            ...
            <div>
              <div class="text-subtitle1 text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">Task Breakdown</div>
              <div class="text-caption" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">Manage deliverables, statuses, assignees, and deadlines</div>
            </div>
            <q-badge color="primary" rounded :label="`${filteredTasks.length} tasks`" class="q-ml-sm" />
          </div>

          <q-btn unelevated no-caps color="primary" icon="add" label="New Task" class="text-weight-bold" @click="showCreateTaskDialog = true" />
        </q-card-section>
        ...
        <q-card-section class="row items-center q-gutter-xs wrap">
          <q-input v-model="taskSearch" outlined dense clearable placeholder="Search tasks..." style="min-width: 180px">
            <template #prepend><q-icon name="search" size="18px" /></template>
          </q-input>

          <q-select v-model="statusFilter" outlined dense emit-value map-options :options="taskStatusOptions" label="Status" style="width: 130px" />
          <q-select v-model="priorityFilter" outlined dense emit-value map-options :options="taskPriorityOptions" label="Priority" style="width: 130px" />
          <q-select v-model="assigneeFilter" outlined dense emit-value map-options :options="assigneeOptions" label="Assignee" style="width: 140px" />

          <q-btn flat dense no-caps icon="refresh" label="Reset" :color="$q.dark.isActive ? 'grey-4' : 'grey-7'" @click="resetTaskFilters" />
        </q-card-section>
        ...
        >
          <template #body-cell-title="props">
            <q-td :props="props">
              <div class="row items-center no-wrap q-gutter-xs">
                <q-icon :name="getPriorityIcon(props.row.priority)" size="16px" :color="props.row.priority === 'CRITICAL' ? 'negative' : props.row.priority === 'HIGH' ? 'warning' : 'primary'" />
                <div>
                  <div class="text-weight-bold ellipsis" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">{{ props.row.title }}</div>
                  <div v-if="props.row.description" class="text-caption ellipsis" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'" style="max-width: 280px">{{ props.row.description }}</div>
                </div>
              </div>
            </q-td>
          </template>
          ...
          <template #body-cell-assignee="props">
            <q-td :props="props">
              <div v-if="getAssigneeMembers(props.row).length" class="row items-center q-gutter-xs">
                ...
                >
                  <q-avatar size="18px" color="primary" text-color="white">{{ member.name.charAt(0) }}</q-avatar>
                  <span class="q-ml-xs">{{ member.name }}</span>
                </q-chip>
              </div>
              <div v-else class="row items-center q-gutter-xs text-caption" :class="$q.dark.isActive ? 'text-grey-3' : 'text-dark'">
                <q-avatar size="22px" color="primary" text-color="white" class="text-weight-bold">{{ getAssigneeName(props.row).charAt(0) }}</q-avatar>
                <span>{{ getAssigneeName(props.row) }}</span>
              </div>
            </q-td>
          </template>
          ...
          <template #body-cell-progress="props">
            <q-td :props="props">
              <div class="row items-center no-wrap q-gutter-xs">
                <q-linear-progress rounded size="6px" :value="getTaskProgressNumber(props.row.progress) / 100" color="primary" class="col" style="min-width: 60px" :track-color="$q.dark.isActive ? 'grey-9' : 'grey-3'" />
                <span class="text-caption text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">{{ getTaskProgressNumber(props.row.progress) }}%</span>
              </div>
            </q-td>
          </template>
          ...
          <template #body-cell-deadline="props">
            <q-td :props="props">
              <div class="row items-center no-wrap text-caption" :class="isTaskOverdue(props.row) ? 'text-negative text-weight-bold' : ($q.dark.isActive ? 'text-grey-4' : 'text-grey-7')">
                <q-icon :name="isTaskOverdue(props.row) ? 'warning' : 'event'" size="14px" class="q-mr-xs" />
          ...
          <template #body-cell-effort="props">
            <q-td :props="props">
              <span class="text-caption text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">
                {{ Number(props.row.actual_effort) || 0 }}h / {{ Number(props.row.expected_effort) || 0 }}h
              </span>
            </q-td>
          </template>
          ...
          <template #body-cell-actions="props">
            <q-td :props="props" auto-width>
              <q-btn flat round dense icon="more_horiz" :color="$q.dark.isActive ? 'grey-4' : 'grey-6'">
                <q-menu auto-close>
                  <q-list style="min-width: 140px">
                    <q-item clickable @click="openQuickUpdate(props.row)"><q-item-section avatar><q-icon name="edit_note" size="18px" color="primary" /></q-item-section><q-item-section>Update Progress</q-item-section></q-item>
                    <q-item clickable @click="openDependencyDialog(props.row)"><q-item-section avatar><q-icon name="account_tree" size="18px" color="primary" /></q-item-section><q-item-section>Add Dependency</q-item-section></q-item>
                    <q-item clickable @click="toggleTaskComplete(props.row)"><q-item-section avatar><q-icon :name="props.row.status === 'COMPLETED' ? 'replay' : 'check_circle'" size="18px" :color="props.row.status === 'COMPLETED' ? 'orange' : 'positive'" /></q-item-section><q-item-section>{{ props.row.status === 'COMPLETED' ? 'Mark Incomplete' : 'Mark Complete' }}</q-item-section></q-item>
                    <q-separator />
                    <q-item clickable class="text-negative" @click="confirmDeleteTask(props.row)"><q-item-section avatar><q-icon name="delete" size="18px" color="negative" /></q-item-section><q-item-section>Delete Task</q-item-section></q-item>
                  </q-list>
                </q-menu>
              </q-btn>
            </q-td>
          ...
          <q-card flat bordered :dark="$q.dark.isActive" class="rounded-borders full-height">
            <q-card-section class="row items-center justify-between">
              <div class="row items-center">
                ...
                <div class="text-subtitle1 text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">Assigned Team & Workload</div>
              </div>
              <div class="row items-center q-gutter-xs">
                <q-badge color="primary" rounded :label="`${teamMembers.length} Members`" />
                <q-btn outline dense no-caps icon="person_add" label="Add" color="primary" class="q-px-xs text-caption text-weight-bold" @click="openAddMemberDialog" />
              </div>
            </q-card-section>
            ...
            <q-list separator :dark="$q.dark.isActive">
              <q-item v-for="member in teamMembers" :key="member.id" class="q-py-md">
                <q-item-section avatar>
                  <q-avatar size="36px" color="primary" text-color="white" class="text-weight-bold">{{ member.name.charAt(0) }}</q-avatar>
                </q-item-section>

                <q-item-section>
                  <q-item-label class="text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">{{ member.name }}</q-item-label>
                  <q-item-label caption :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">{{ member.role }}</q-item-label>
                </q-item-section>

                <q-item-section side>
                  <div class="row items-center q-gutter-xs">
                    <q-chip dense square color="primary" text-color="white" class="text-caption text-weight-bold">{{ member.assignedTasks }} Tasks</q-chip>
                    <div style="width: 80px">
                      <q-linear-progress rounded size="4px" :value="member.capacity / 100" :color="member.capacity > 80 ? 'orange' : 'primary'" :track-color="$q.dark.isActive ? 'grey-9' : 'grey-3'" />
                      <div class="text-caption text-grey-5 text-center" style="font-size: 9px">{{ member.capacity }}% capacity</div>
                    </div>
                    <q-btn flat round dense icon="person_remove" :color="$q.dark.isActive ? 'grey-4' : 'grey-6'" size="sm" @click="confirmRemoveProjectMember(member)" />
                  </div>
                </q-item-section>
              </q-item>
            </q-list>
          ...
          <q-card flat bordered :dark="$q.dark.isActive" class="rounded-borders full-height">
            <q-card-section class="row items-center justify-between">
              <div class="row items-center">
                ...
                <div class="text-subtitle1 text-weight-bold" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">Recent Activity</div>
              </div>
              <span class="text-caption" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">Live Stream</span>
            </q-card-section>
            ...
            <q-list separator :dark="$q.dark.isActive">
              <q-item v-for="act in activityLogs" :key="act.id" class="q-py-md">
                <q-item-section avatar>
                  <q-avatar size="28px" :color="act.type === 'complete' ? 'positive' : act.type === 'create' ? 'info' : 'primary'" text-color="white">
                ...
                <q-item-section>
                  <div class="row items-center justify-between">
                    <span class="text-weight-bold text-caption" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">{{ act.user }}</span>
                    <span class="text-caption" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-6'">{{ act.time }}</span>
                  </div>
                  <div class="text-caption q-mt-xs" :class="$q.dark.isActive ? 'text-grey-3' : 'text-grey-8'">{{ act.message }}</div>
                </q-item-section>
              </q-item>
            </q-list>
          </q-card>
        ...
        <q-card :dark="$q.dark.isActive" style="width: 480px; max-width: 95vw; border-radius: 12px">
          ...
          <q-form @submit.prevent="handleAddDependency">
            <q-card-section class="column q-gutter-y-sm">
              <q-input :model-value="dependencyTaskLabel" outlined dense readonly label="Task" :dark="$q.dark.isActive" />
            ...
            <q-card-actions align="right" class="q-pa-md">
              ...
              <q-btn type="submit" no-caps unelevated label="Add Dependency" color="primary" :loading="dependencySubmitting" :disable="!selectedDependencyTaskId || !selectedPredecessorTaskIds?.length" />
            </q-card-actions>
          </q-form>
        </q-card>
      </q-dialog>
      ...
      <q-dialog v-model="showEditProjectDialog">
        <q-card :dark="$q.dark.isActive" style="width: 520px; max-width: 95vw; border-radius: 12px">
          ...
          <q-form @submit.prevent="handleSaveProject">
            <q-card-section class="column q-gutter-y-sm">
              <q-input v-model="editProjectForm.name" outlined dense label="Project Name *" :dark="$q.dark.isActive" :rules="[(val) => !!val.trim() || 'Project name is required']" />
              <q-input v-model="editProjectForm.description" outlined dense type="textarea" label="Description" autogrow :dark="$q.dark.isActive" />
              <div class="row q-col-gutter-sm">
                <div class="col-6">
                  <q-select v-model="editProjectForm.status" outlined dense label="Status" :options="projectStatusOptions" emit-value map-options :dark="$q.dark.isActive" />
                </div>
                <div class="col-6">
                  <q-select v-model="editProjectForm.priority" outlined dense label="Priority" :options="projectPriorityOptions" emit-value map-options :dark="$q.dark.isActive" />
                </div>
              </div>
              <div class="row q-col-gutter-sm">
                <div class="col-6">
                  <q-input v-model="editProjectForm.start_date" outlined dense type="date" label="Start Date" stack-label :dark="$q.dark.isActive" />
                </div>
                <div class="col-6">
                  <q-input v-model="editProjectForm.deadline" outlined dense type="date" label="Deadline" stack-label :dark="$q.dark.isActive" />
                </div>
              </div>
            </q-card-section>
          ...
          <q-card-section v-if="selectedTaskForUpdate" class="column q-gutter-y-sm">
            <div>
              ...
              <div class="row q-gutter-xs q-mt-xs">
                <q-btn
                  ...
                  no-caps
                  unelevated
                  :color="selectedTaskForUpdate.status === st.value ? 'primary' : ($q.dark.isActive ? 'grey-9' : 'grey-3')"
                  :text-color="selectedTaskForUpdate.status === st.value ? 'white' : ($q.dark.isActive ? 'grey-3' : 'dark')"
                  :label="st.label"
            ...
            <div>
              <div class="row justify-between items-center">
                ...
                <span class="text-caption text-weight-bold text-primary">{{ selectedTaskForUpdateProgress }}%</span>
              </div>
              <q-slider v-model="selectedTaskForUpdateProgress" :min="0" :max="100" :step="5" color="primary" label />
            </div>

            <div>
              <span class="text-caption text-weight-bold">Logged Effort (Hours)</span>
              <q-input v-model.number="selectedTaskForUpdate.actual_effort" outlined dense type="number" min="0" step="0.5" :dark="$q.dark.isActive" class="q-mt-xs" />
            </div>
          </q-card-section>
        ...
        <q-card :dark="$q.dark.isActive" style="width: 480px; max-width: 95vw; border-radius: 12px">
          ...
          <q-card-section>
            <q-select
              ...
              map-options
              :dark="$q.dark.isActive"
              :display-value="selectedMembersToAdd.length ? `${selectedMembersToAdd.length} selected` : ''"
          ...
          <q-card-actions align="right" class="q-pa-md">
            <q-btn v-close-popup flat no-caps label="Cancel" />
            <q-btn no-caps unelevated color="primary" label="Add to Project" :loading="addingMember" :disable="!selectedMembersToAdd.length" @click="handleAddProjectMember" />
          </q-card-actions>
        </q-card>
      </q-dialog>
      ...
      >
        Are you sure you want to unassign <strong>{{ unassignTaskTarget.resourceName }}</strong> from task <strong>"{{ unassignTaskTarget.taskTitle }}"</strong>?
      </ConfirmActionDialog>
...
const dependencyTaskLabel = computed(() => {
  ...
  const found = tasks.value.find((t) => Number(t.task_id) === Number(selectedDependencyTaskId.value));
...
const taskColumns: QTableColumn<Task>[] = [
  { name: 'title', label: 'Task Details', field: (row) => row.title, align: 'left', sortable: true },
  { name: 'status', label: 'Status', field: (row) => row.status, align: 'left', sortable: true },
  { name: 'priority', label: 'Priority', field: (row) => row.priority, align: 'left', sortable: true },
  { name: 'assignee', label: 'Assignee', field: (row) => getAssigneeName(row), align: 'left' },
  { name: 'progress', label: 'Progress', field: (row) => getTaskProgressNumber(row.progress), align: 'left', sortable: true },
  { name: 'deadline', label: 'Deadline', field: (row) => row.deadline ?? '', align: 'left', sortable: true },
  { name: 'effort', label: 'Effort (Actual/Plan)', field: (row) => Number(row.expected_effort) || 0, align: 'left' },
...
const healthColor = computed(() => {
  if ($q.dark.isActive) {
    return projectHealth.value === 'AT_RISK' ? 'orange-10' : projectHealth.value === 'DELAYED' ? 'red-10' : 'green-10';
  }
  return projectHealth.value === 'AT_RISK' ? 'orange-1' : projectHealth.value === 'DELAYED' ? 'red-1' : 'green-1';
});

const healthTextColor = computed(() => {
  if ($q.dark.isActive) {
    return projectHealth.value === 'AT_RISK' ? 'orange-2' : projectHealth.value === 'DELAYED' ? 'red-2' : 'green-2';
  }
  return projectHealth.value === 'AT_RISK' ? 'orange-9' : projectHealth.value === 'DELAYED' ? 'red-8' : 'green-8';
});
...
const pendingTasksCount = computed(() => tasks.value.filter((t) => t.status === 'UNASSIGNED' || t.status === 'SCHEDULED').length);
...
const totalEffortExpected = computed(() => tasks.value.reduce((sum, t) => sum + (Number(t.expected_effort) || 0), 0));
const totalEffortLogged = computed(() => tasks.value.reduce((sum, t) => sum + (Number(t.actual_effort) || 0), 0));
const completedMilestonesCount = computed(() => milestones.value.filter((m) => m.progress === 100).length);
const activeAssigneesCount = computed(() => teamMembers.value.filter((m) => m.assignedTasks > 0).length);
...
const filteredTasks = computed(() => {
  ...
  return tasks.value.filter((task) => {
    ...
    const matchesAssignee = assigneeFilter.value === 'ALL' || (task.assigned_resource_ids && task.assigned_resource_ids.includes(Number(assigneeFilter.value)));
...
function priorityColor(priority: string | null | undefined): string {
  ...
  switch (priority) {
    case 'LOW': return 'blue-1';
    case 'MEDIUM': return 'deep-purple-1';
...
function statusColor(status: string | null | undefined): string {
  ...
  switch (status) {
    case 'SCHEDULED': return 'deep-purple-1';
...
function getAssigneeName(task: Task): string {
  if (task.assigned_resource_ids && task.assigned_resource_ids.length > 0) {
    const names = task.assigned_resource_ids.map((id) => teamMembers.value.find((m) => m.id === id)?.name).filter(Boolean);
...
function updateDerivedMilestones() {
  milestones.value = tasks.value
    ...
    .map((task) => {
      ...
      const status: Milestone['status'] = task.status === 'COMPLETED' ? 'COMPLETED' : task.status === 'IN_PROGRESS' ? 'IN_PROGRESS' : 'UPCOMING';
      return {
        id: task.task_id,
        name: task.title,
        status,
...
async function updateActivityLogs() {
  try {
    ...
    const projectLogs = feedLogs.filter(
      (log) => (log.task_id && projectTaskIds.has(log.task_id)) || (log.project_name && project.name && log.project_name.toLowerCase() === project.name.toLowerCase()),
    );
...
function confirmDeleteProject() { showDeleteProjectDialog.value = true; }
...
function confirmDeleteTask(task: Task) { taskToDelete.value = task; showDeleteTaskDialog.value = true; }
...
async function handleExecuteUnassignTask() {
  ...
  } finally { unassigningTask.value = false; }
}

function confirmRemoveProjectMember(member: TeamMember) { memberToRemove.value = member; showRemoveMemberDialog.value = true; }
...
async function loadProjectTeamMembers() {
  try {
    ...
    teamMembers.value = fetchedProjectMembers.map((resource) => {
      const assigned = tasks.value.filter((task) => task.assigned_resource_ids?.includes(resource.user_id));
  ...
  } finally { void updateActivityLogs(); }
}
...
async function handleAddDependency() {
  ...
  } finally { dependencySubmitting.value = false; }
}
