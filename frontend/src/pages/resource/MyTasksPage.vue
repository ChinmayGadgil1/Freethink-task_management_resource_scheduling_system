      ...
      <div class="row items-center justify-between q-gutter-md">
        <div>
          <div class="row items-center q-gutter-sm">
            <q-avatar size="42px" color="primary" text-color="white" icon="task_alt" />
    ...
    <q-card flat bordered class="dashboard-card q-mt-md">
      ...
      <q-card-section class="q-py-md">
        <div class="row items-center justify-between q-gutter-md">
          <div>
            <div class="row items-center q-gutter-sm">
              <q-avatar size="36px" color="primary" text-color="white" icon="assignment" />
      ...
      <q-card-section class="q-py-md">
        <div class="row items-center q-col-gutter-md">
          ...
          <div v-if="hasActiveFilters" class="col-auto">
            <q-chip dense color="blue-1" text-color="primary" icon="filter_alt">
              Filters active
            </q-chip>
          </div>
        </div>
      ...
      <div v-if="loading" class="q-pa-xl text-center">
        ...
        <div class="text-body2 text-grey-6 q-mt-md">Loading your tasks...</div>
      </div>

      <!-- ERROR -->
      <div v-else-if="error" class="q-pa-xl text-center">
        <q-avatar size="56px" color="red-1" text-color="negative" icon="error_outline" />
      ...
      <div v-else-if="filteredTasks.length === 0" class="q-pa-xl text-center">
        ...
        <div class="text-body1 text-weight-medium text-grey-7 q-mt-md">No tasks found</div>
      ...
      <template v-else>
        <div class="q-py-sm">
          <div v-for="task in paginatedTasks" :key="task.id" class="q-px-md q-py-xs">
            <TaskListItem :task="task" @update="openUpdateDialog" />
          </div>
        </div>
        ...
        <q-card-section class="q-py-md">
          <div class="row items-center justify-between q-gutter-md">
            <div class="text-caption text-grey-6">
              Showing
              {{ Math.min((currentPage - 1) * rowsPerPage + 1, filteredTasks.length) }}
              -
...
async function loadTasks() {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : 'Failed to load your tasks.';
  } finally {
    loading.value = false;
  }
}
...
const statCards = computed(() => [
  ...
  {
    title: 'In Progress',
    value: tasks.value.filter((task) => task.status === 'IN_PROGRESS').length,
    icon: 'autorenew',
    color: '#2E90FA',
  },
...
const projectOptions = computed(() => [...new Set(tasks.value.map((task) => task.project))].sort());
...
const filteredTasks = computed(() =>
  tasks.value.filter((task) => {
    ...
    const matchesSearch =
      !q || task.name.toLowerCase().includes(q) || task.project.toLowerCase().includes(q);

    const matchesStatus =
      !statusFilter.value ||
      (statusFilter.value === 'DELAYED' ? isOverdue(task) : task.status === statusFilter.value);
...
//SORTING
type SortOption =
  'TITLE_ASC' | 'TITLE_DESC' | 'PRIORITY' | 'STATUS' | 'DEADLINE' | 'PROGRESS' | 'HOURS';

const sortBy = ref<SortOption>('DEADLINE');
...
const statusOrder: Record<ResourceTask['status'], number> = {
  IN_PROGRESS: 1,
  PENDING: 2,
  ON_HOLD: 3,
  COMPLETED: 4,
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(sortedTasks.value.length / rowsPerPage.value)),
);
...
watch([search, statusFilter, projectFilter, priorityFilter, sortBy, rowsPerPage], () => {
  currentPage.value = 1;
});
